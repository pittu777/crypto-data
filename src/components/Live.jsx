...
const Live = () => {
  ...
  return (
    <div className="flex flex-col mt-9 border border-gray-100 rounded">
      {loading && <p>Loading...</p>}
      {!loading && Object.keys(realTimeData).length > 0 && (
        <table className="w-full table-auto">
          ...
          <tbody>
            {cryptoData && cryptoData.length > 0 && cryptoData.map(data => {
              const { id, symbol, name, total_volume, market_cap_change_percentage_24h, price_change_percentage_1h_in_currency, price_change_percentage_24h_in_currency, price_change_percentage_7d_in_currency } = data;
              const price = realTimeData[symbol];
              if (!price) return null;
              return (
                <tr key={id} className="text-base border-b border-gray-100 hover:bg-gray-200 last:border-b-0">
                  ...
                  <td className="py-4">{total_volume}</td>
                  <td className="py-4">{market_cap_change_percentage_24h}%</td>
                  <td className={price_change_percentage_1h_in_currency > 0 ? "text-green py-4" : "text-red py-4"}>{Number(price_change_percentage_1h_in_currency).toFixed(2)}</td>
                  <td className={price_change_percentage_24h_in_currency > 0 ? "text-green py-4" : "text-red py-4"}>{Number(price_change_percentage_24h_in_currency).toFixed(2)}</td>
                  <td className={price_change_percentage_7d_in_currency > 0 ? "text-green py-4" : "text-red py-4"}>{Number(price_change_percentage_7d_in_currency).toFixed(2)}</td>
                </tr>
              )
            })}
          </tbody>
